...
**Research Harness boundary (recorded):** Note Skills is not a replacement for an upstream Research Control Plane. It consumes trusted facts from a `canonical_state_file` adapter (milestones / noteStatuses / canonical_conflicts) and never infers them itself; it does not implement Research Event streams, materialized research state, ExperimentSpec/RunManifest, TaskSlice/Handoff, artifact hashing, Definition revision/impact analysis, or a rebuildable SQLite structured index. The adapter contract is documented in `Note_Skills_Design.md` §6.3.
